...
def plot_stock_price(ticker):
    try:
        ...
        if not data.empty:
            ...
            plt.title(f'{ticker} 1-Year Stock Price Chart')
            plt.xlabel('Date')
            plt.ylabel('Price (USD)')
            plt.savefig('stock.png', bbox_inches='tight')
            return "The 1-year stock price chart for the given ticker has been generated."
        else:
            return "Unable to fetch stock price data for the given ticker symbol."
...
def compare_stock_sp500(ticker):
    try:
        ...
        if not stock_data.empty and not sp500_data.empty:
            stock_return = (stock_data['Close'][-1] - stock_data['Close'][0]) / stock_data['Close'][0] * 100
...
def get_stock_type(ticker):
    data = yf.Ticker(ticker).info
    if 'trailingPE' in data and 'forwardPE' in data and 'priceToBook' in data and 'industry' in data:
        trailing_pe = data['trailingPE']
        forward_pe = data['forwardPE']
        price_to_book = data['priceToBook']
        industry = data['industry']

        if trailing_pe > 20 and forward_pe > 20 and industry.lower() in ['technology', 'software', 'internet', 'biotechnology']:
            return f"{ticker} is considered a growth stock based on its high P/E ratios and industry ({industry})."
        elif price_to_book < 1.5 and industry.lower() in ['banking', 'insurance', 'energy', 'utilities']:
    ...
    try:
        ...
        if 'longBusinessSummary' in company_info:
            ...
            if 'riskFactors' in company_info and company_info['riskFactors']:
                risk_factors = company_info['riskFactors'][:3]  # Get the top 3 risk factors

            if risk_factors:
                risk_summary = '\n'.join([f"{idx + 1}. {risk}" for idx, risk in enumerate(risk_factors)])
    ...
    try:
        ...
        if company_info.empty:
            ...
            if earnings_dates.empty:
                ...
                formatted_date = datetime.utcfromtimestamp(next_earnings_date).strftime('%Y-%m-%d')
    ...
    try:
        ...
        if data.empty:
            ...
            for i in range(len(close_prices) - 2):
                # Left shoulder
                if not left_shoulder_found and close_prices[i] < close_prices[i + 1] > close_prices[i + 2]:
                    left_shoulder_idx = i + 1
                    left_shoulder_found = True
                    continue

                # Head
                if left_shoulder_found and not head_found and close_prices[i] < close_prices[i + 1] > close_prices[i + 2]:
                    head_idx = i + 1
                ...
                # Right shoulder
                if head_found and not right_shoulder_found and close_prices[i] < close_prices[i + 1] > close_prices[i + 2]:
                    right_shoulder_idx = i + 1
                    if highs[right_shoulder_idx] < highs[head_idx] and lows[right_shoulder_idx] > lows[left_shoulder_idx]:
...
def compare_valuations(ticker1, ticker2):
    try:
        ...
        metrics = [
            ('Market Cap (B)', 'marketCap', lambda x: f"${x / 1e9:.2f}B" if x else 'N/A'),
            ('Enterprise Value (B)', 'enterpriseValue', lambda x: f"${x / 1e9:.2f}B" if x else 'N/A'),
...
def get_sector_stocks(sector):
    try:
        ...
        sector_stocks = [ticker for ticker, info in tickers.info.items() if info.get('sector') == sector]
...
def get_top_sector_stocks(sector, period='1mo', top_n=10):
    try:
        ...
        sector_map = {
            ...
            'Consumer Discretionary': {'etf': 'XLY', 'name': 'Consumer Discretionary Select'},
            ...
            'Communication Services': {'etf': 'XLC', 'name': 'Communication Services Select'},
        ...
        if sector == 'Technology':
            sector_stocks = [
                'AAPL', 'MSFT', 'AMZN', 'NVDA', 'META', 'GOOGL', 'GOOG', 'TSLA', 'AVGO', 'CSCO',
                'ADBE', 'INTC', 'NFLX', 'PYPL', 'CMCSA', 'AMD', 'QCOM', 'INTU', 'TXN', 'AMAT'
            ]
        else:
            # Fetch top holdings from the sector ETF
            sector_etf = yf.Ticker(etf_ticker)
            sector_stocks = [stock[0] for stock in sector_etf.info.get('holdings', [])[:20]]
        ...
        time_descriptions = {'1mo': 'month', '3mo': '3 months', '6mo': '6 months', '1y': 'year'}
...
def get_potential_growth_stocks(industry, top_n=5):
    ...
    indices = ['^GSPC', '^IXIC', '^DJI']  # S&P 500, Nasdaq Composite, Dow Jones Industrial Average
        ...
        for ticker in tickers:
            try:
                ...
                if 'industry' in info and info['industry'] == industry:
                    ...
                    if (trailing_pe > 25 or forward_pe > 25) and (earnings_growth > 0.15 or revenue_growth > 0.15) and beta > 1:
    ...
    for stock in potential_stocks:
        stock['Growth Score'] = (
            0.3 * (stock['Earnings Growth'] if stock['Earnings Growth'] != float('inf') else 0) +
            0.3 * (stock['Revenue Growth'] if stock['Revenue Growth'] != float('inf') else 0) +
            0.2 * (1 / stock['PEG Ratio'] if stock['PEG Ratio'] not in [0, float('inf')] else 0) +
...
def calculate_portfolio_performance(period='1y'):
    ...
    for ticker, shares in st.session_state.portfolio.items():
        try:
            ...
            if not data.empty and len(data) > 1:
                ...
                st.warning(f"Insufficient data for {ticker}. Skipping from calculation.")
        except Exception as e:
            st.error(f"Error fetching data for {ticker}: {e}")
    ...
    if total_initial_value > 0:
        ...
        if len(weighted_returns) > 6:
            response += "   ...\n"
            for ticker, return_pct, weight in weighted_returns[-3:]:  # Bottom 3 detractors
                contribution = return_pct * weight
                response += f"   - {ticker}: {return_pct:.2f}% | Weight: {weight:.1%} | Contribution: {contribution:.2f}%\n"
        ...
        try:
            ...
            if not data.empty:
                ...
                if sector:
                    ...
                    st.warning(f"Unable to determine sector for {ticker}. It will be categorized as 'Unknown'.")
            else:
...
def should_rebalance_portfolio():
    ...
    allocations = {ticker: value / total_value for ticker, value in stock_values.items()}
    ...
    for ticker, allocation in allocations.items():
        if abs(allocation - equal_weight) > threshold:
            return ("Yes, you should consider rebalancing your portfolio. Some stocks have deviated "
...
def get_latest_market_news():
    try:
        ...
        if market_news['totalResults'] > 0:
            ...
            return f"The latest news affecting the stock market is: {title}\n{description}"
        else:
...
def get_top_trending_stocks():
    try:
        ...
        newsapi = NewsApiClient(api_key=news_api_key)
        trending_stocks = newsapi.get_everything(q='trending stocks', language='en', sort_by='relevancy')
        if trending_stocks['totalResults'] > 0:
            trending_articles = trending_stocks['articles'][:5]
            trending_stocks_list = [article['title'].replace('\n', ' ') for article in trending_articles]
...
def get_market_valuation():
    try:
        ...
        newsapi = NewsApiClient(api_key=news_api_key)
        market_valuation = newsapi.get_everything(q='stock market valuation', language='en', sort_by='relevancy')
...
def get_economic_events_impacting_market():
    try:
        ...
        newsapi = NewsApiClient(api_key=news_api_key)
        economic_events = newsapi.get_everything(q='economic events stock market', language='en', sort_by='relevancy')
...
def get_analyst_opinions(ticker):
    try:
        ...
        newsapi = NewsApiClient(api_key=news_api_key)
        analyst_opinions = newsapi.get_everything(q=f"{ticker} analyst opinions", language='en', sort_by='relevancy')
...
functions = [
...

    {
        ...
        'description': "Generate a 1-year stock price chart for a given ticker symbol.",
        'parameters': {
            'type': 'object',
            'properties': {
                'ticker': {
...
def get_response(query, chat_history):
    st.session_state['messages'] = [
        {'role': 'system', 'content': 'You are a helpful financial advisor named prabot, that explains concepts in an easy-to-understand way.'}
    ]
    ...
    if user_input:
        try:
            ...
            if response_message.function_call:
                ...
                args_dict = {}
                function_to_call = available_functions[function_name]  # Move this line to the top
                ...
                if function_name == 'plot_stock_with_macd':
                    ...
                    if function_name in ['get_stock_price', 'calculate_RSI', 'calculate_MACD', 'plot_stock_price']:
                        ...
                        args_dict = {'ticker': function_args.get('ticker')}
                    elif function_name == 'get_stock_open_price':  # Fix for get_stock_open_price
                        args_dict = {'ticker': function_args.get('ticker')}
                    ...
                    #    return st.session_state['messages']
                        ...
                        function_response = function_to_call(**args_dict)
                    elif function_name == 'get_top_sector_stocks':  # New condition for your function
                        args_dict = {
                        ...
                        function_response = function_to_call(**args_dict)
                    elif function_name == 'get_potential_growth_stocks':  # New condition for your function
                        args_dict = {
...
#                        args_dict = {} 
                    ...
                    elif function_name in ['get_best_performing_stock', 'analyze_sector_allocation']:
                        args_dict = {}
